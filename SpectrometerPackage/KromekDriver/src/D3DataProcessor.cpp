...
// Note: _inputBufferSection should be locked by calling method
...
{
	kmk::Lock lock(_inputBufferSection);
	...
	while (1)
	{
		...
		if (totalDataLength - readIndex >= 2)
		{
			...
			// Check for a sign that the data is corrupted
...
// NOTE: _inputBufferSection should be locked BEFORE calling this function
...
{
	kmk::Lock lock(_criticalSection);
	kmk::Lock lock2(_inputBufferSection);
...
bool D3DataProcessor::GetConfigurationData(uint8_t componentId, uint16_t configurationIds, BYTE *pDataOut, size_t &dataLength)
{
	...
	if ((configurationIds & REPORT_MASK_USE_PARENT) || configurationId == REPORT_ID_GET_STATUS || configurationId == REPORT_ID_GET_DEVICE_INFO)
...
int D3DataProcessor::ProcessThreadProc(void *pArg)
{
	...
	do
	{
		...
		if (kmk::Time::GetTimeMs() >= nextQueryTime)
		{
            ...
            bool configOnly = (pThis->_neutronComponent.status == TS_STOP && pThis->_gammaComponent.status == TS_STOP);
			if (!configOnly)
			{
...

	{
		...
		if (pThis->_gammaComponent.finishedCallback != NULL && pThis->_gammaComponent.status != TS_STOP)
		{
			...
			(*(pThis->_gammaComponent.finishedCallback))(pThis->_gammaComponent.finishedCallbackArg, forcedStop);
		}
		...
		if (pThis->_neutronComponent.finishedCallback != NULL && pThis->_neutronComponent.status != TS_STOP)
		{
			...
			(*(pThis->_neutronComponent.finishedCallback))(pThis->_neutronComponent.finishedCallbackArg, forcedStop);
		}
	}
...
void D3DataProcessor::RaiseError(int errorCode, String message)
{
	...
	if (_gammaComponent.status != TS_STOP)
	{
		...
		{
			(*_gammaComponent.errorCallback)(_gammaComponent.errorCallbackArg, errorCode, message);
		}
	}
	...
	if (_neutronComponent.status != TS_STOP)
	{
		...
		{
			(*_neutronComponent.errorCallback)(_neutronComponent.errorCallbackArg, errorCode, message);
		}
	}
}
