...
bool SerialDataInterface::SendDataToDevice(OVERLAPPED &overlap, const BYTE *pData, size_t dataSize)
{
	...
	while (bytesSent < dataSize)
	{
		...
		WriteFile(_fileHandle, &pData[bytesSent], dataSize - bytesSent, NULL, &overlap);
		...
		if (!success)
		{
			...
			swprintf_s(msgBuffer, 200, L"Write Data Error - ErrorCode = %d. Written = %d, Expected %d", GetLastError(), bytesWritten, dataSize - bytesSent);
