...
int stdcall kr_BeginDataAcquisitionF(unsigned int deviceID, unsigned int realTime, unsigned int liveTime, int64_t *ts_out, double A0, double A1, double A2,char fileName[])
{
  //printf("test arguments %s \n",fileName);
  int64_t ts=0;
  int res=DriverMgr::GetInstance()->BeginDataAcquisitionF(deviceID, realTime, liveTime, &ts,A0,A1,A2,fileName);
  //printf("test Driver arguments %ld \n",ts);
  *ts_out=ts;
  return res;
}
